...
def update_from_jwc_to_sqlite(username, password):
    session = requests.Session()
    login_ret = login(session, username, password)
    while True:
        ...
        if loginResponse == "success":
            ...
            print("登陆失败！")
            time.sleep(2)
    courses_text = session.get(url=already_select_course_url, headers=header).text
